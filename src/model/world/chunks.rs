...
impl Chunks {
    pub fn new(size: Vec2, cell_size: f32) -> Self {
        ...
        Chunks {
            grid,
            world_size: size,
            grid_size: vec2(grid_width as f32, grid_height as f32),
        }
    }
    pub fn get(&self, pos: Vec2) -> &Chunk {
        let y = ((pos.y / self.world_size.y * self.grid_size.y) as usize)
            .clamp(0, self.grid_size.y as usize - 1);
        let x = ((pos.x / self.world_size.x * self.grid_size.x) as usize)
            .clamp(0, self.grid_size.x as usize - 1);
        &self.grid[y * self.grid_size.y as usize + x]
    }
}
